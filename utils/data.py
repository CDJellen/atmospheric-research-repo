...
def get_cn2(data_dir=DATA_IN, dropna=True, drop_above=5e-12):
    ...
    <docstring>
    """
    df_sc = pd.read_excel(os.path.join(DATA_IN, "Cn2_Data_Scintillometer.xlsx"),
                          skiprows=np.arange(101),
                          header=None,
                          parse_dates=True,
                          dtype={
                              'A': np.datetime64,
    ...
    df_sc_dlst_mask = (df_sc['Timestamp'] >= dt.datetime(
        2020, 3, 8, 2)) & (df_sc['Timestamp'] <= dt.datetime(2020, 11, 1, 2))
    df_sc['Timestamp'].loc[df_sc_dlst_mask] = df_sc['Timestamp'].loc[
        df_sc_dlst_mask] + dt.timedelta(hours=1)
    df_sc = df_sc.set_index('Timestamp')
    return df_sc


def get_wx(data_dir=DATA_IN,
           dropna=True,
           skiprows=np.arange(2),
           usecols="A,D,G:M,R,U,X,AH",
           na_values={'---', '------'}):
    """
    <doctring>
    """
    df_wx = pd.read_excel(os.path.join(
        DATA_IN, "01_JAN_to_15JUNE2020_local_station_weather.xlsx"),
                          engine='openpyxl',
                          skiprows=skiprows,
                          header=None,
                          usecols=usecols,
    ...
    df_wx.columns = [
        ...
        "Dew_Point_Temperature", "Wind_Speed", "Wind_Direction", "Wind_Run",
        "Wind_Speed_Gust", "Wind_Direction_Gust", "Pressure", "Solar_Radiation",
        "UV_Index", "Air_Density"
    ]
    df_wx = df_wx.set_index('Timestamp')
    return df_wx


def get_ndbc(data_dir=DATA_IN,
             dropna=True,
             skiprows=np.arange(2),
             usecols="A:E,N,O",
             na_values=[99, 999]):
    ...
    <docstring>
    """
    df_ndbc = pd.read_excel(os.path.join(DATA_IN, "NDBC-TPLM2-Data.xlsx"),
                            header=None,
                            na_values=na_values,
                            usecols=usecols,
                            skiprows=skiprows,
                            ...
                            engine='openpyxl')
    df_ndbc['Air_Water_Temperature_Difference'] = df_ndbc[
        'Air_Temperature'] - df_ndbc['Water_Temperature']
    df_ndbc = df_ndbc.drop(columns=['Air_Temperature'])
    df_ndbc = df_ndbc.drop(columns=['Water_Temperature'])
    df_ndbc['Timestamp'] = df_ndbc['Year'].astype(
        str) + '/' + df_ndbc['Month'].astype(str) + '/' + df_ndbc['Day'].astype(
            str) + 'T' + df_ndbc['Hour'].astype(
                str) + ':' + df_ndbc['Minute'].astype(str)
...
def get_sunrise(data_dir=DATA_IN, dropna=True):
    ...
    <docstring>
    """
    df_sunrise = pd.read_excel(os.path.join(
        DATA_IN, "Annapolis_Sunrise_Sunset_Times_2020.xlsx"),
                               header=None,
                               parse_dates=[[0, 1, 2, 3]],
                               engine='openpyxl')
    df_sunrise.columns = ['Sunrise_Time', 'drop']
    df_sunrise = df_sunrise.drop(columns=['drop'])
    df_sunrise['Timestamp'] = (df_sunrise['Sunrise_Time'].dt.date).astype(
        np.datetime64)
...
def get_sunset(data_dir=DATA_IN, dropna=True):
    ...
    <docstring>
    """
    df_sunset = pd.read_excel(os.path.join(
        DATA_IN, "Annapolis_Sunrise_Sunset_Times_2020.xlsx"),
                              header=None,
                              parse_dates=[[0, 1, 2, 4]],
                              engine='openpyxl')
    ...
    df_sunset['Timestamp'] = (df_sunset['Sunset_Time'].dt.date).astype(
        np.datetime64)
...
def enrich_temporal_hour_weight(df):
    ...
    <docstring>
    """
    df['TH'] = (df['Sunset_Time'].values.astype(float) -
                df['Sunrise_Time'].values.astype(float))
    df['Temporal_Hour_Weight'] = (
        df['Timestamp'].values.astype(float) -
        df['Sunrise_Time'].values.astype(float)) * 12 / (df['TH'])
...
def enrich_temporal_hour_weight_oermann(df):
    ...
    df['Solar_Hour_2'] = df['Solar_Hour_2'].where(
        (df['Solar_Hour_2'] >= 0) & (df['Solar_Hour_2'] <= 16), 0)
    df['Temporal_Hour_Weight_Oermann'] = (
        df['Solar_Hour_2'] * 0.3257231 + (df['Solar_Hour_2'].pow(2)) * 0.01738 +
        (df['Solar_Hour_2'].pow(3)) * -0.007102 +
        (df['Solar_Hour_2'].pow(4)) * 0.000305 - 0.5041482)
    df['Temporal_Hour_Weight_Oermann'] = df[
        'Temporal_Hour_Weight_Oermann'].where(
            df['Temporal_Hour_Weight_Oermann'] > 0.1, 0.1)
...
def merge_frames(df_sc, df_wx, df_ndbc, df_sunrise, df_sunset):
    ...
    df = df.merge(df_sunrise, left_index=True, right_index=True,
                  how='outer').fillna(method='ffill')
    df = df.merge(df_sunset, left_index=True, right_index=True,
                  how='outer').fillna(method='ffill')
    df = df.merge(df_ndbc, left_index=True, right_index=True,
                  how='outer').fillna(method='ffill')
...
def validate_df(df, log10=False):
    ...
    df['Wind_Direction_Gust'], uniques_windDirGust = pd.factorize(
        df['Wind_Direction_Gust'])
